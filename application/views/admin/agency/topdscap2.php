                            ...
                            <div class="col-sm-2">
                                <div class="input-group date" id="datetimepicker1">
                                    <input type="text" class="form-control" id="fromMonth" name="fromMonth"
                                           value="<?php echo $this->input->post("fromMonth") ?>"> 
...
<script>
    ...
    $(document).ready(function () {
        //sort table
       $("#fromMonth").val(getFirtDayOfMonth());
            var queryDate = $("#fromMonth").val();
    ...
    function listtopdoanhso(stt,  nickName, total,bonusMore) {
        var html = "";
        html += "<tr>";
    ...
    function listtopdoanhsoAgent(index,  nickName, total,bonusMore) {
        var html = "";
        html += "<tr>";
    ...
    function topDoanhSoAdmin() {
        $.ajax({
            type: "POST",
            url: "<?php echo base_url('TranferAjax/doanhsocap2') ?>",
            data: {
            ...
            success: function (data) {
                ...
                if (data.transactions == "") {
                    ...
                    result += ' <th>Tên đại lý cấp 1</th>';
                    result += ' <th>Doanh số</th>';
                    result += ' <th>Thưởng cố định(<?php echo $namegame ?>))</th>';
                    result += ' </tr>';
                    result += ' </thead>';
                    result += '<tbody>';
                    $.each(data.transactions, function (index, value) {
                        result += listtopdoanhso(index, value.nickName, value.total,value.bonusFix);
                        i++;
                    });
                    result += '</tbody>';
                    result += '</table>';
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.transactions == "") {
                    ...
                    result += '<tbody>';
                    $.each(data.transactions, function (index, value) {
                        result += listtopdoanhsoAgent(index, value.nickName, value.total,value.bonusFix);
                    });
